...
const STATUS_COLOR: Record<string, { dot: string; pill: string }> = {
  ...
  TERMINEE:   { dot: "bg-white/20",    pill: "bg-white/5 text-white/40 border border-white/10" },
    ...
    <div className="space-y-5 animate-slide-up">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white/80 capitalize min-w-[150px] text-center">
          ...
          <button onClick={() => setCurrentDate(new Date())}
            className="px-3 py-1.5 text-xs font-semibold text-white/60 hover:text-white border border-white/[0.08] hover:border-violet-500/40 rounded-xl transition-all bg-white/[0.03] hover:bg-violet-500/10">
            Aujourd&apos;hui
          </button>
        </div>
      </div>
        ...
        <div className="xl:col-span-2 surface overflow-hidden">
          <div className="grid grid-cols-7 border-b border-white/[0.06]">
            {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map(d => (
              <div key={d} className="py-3 text-center text-[11px] font-semibold text-white/25 uppercase tracking-widest">{d}</div>
            ))}
          </div>
            ...
            {days.map((day, idx) => {
              ...
              return (
                <button key={idx}
                  ...
                  className={cn(
                    "relative min-h-[76px] p-2 text-left border-b border-r border-white/[0.04] transition-all duration-150",
                  ...
                  <span className={cn(
                    ...
                    !isTodayDate && isCurrentMonth && "text-white/60",
                    !isTodayDate && !isCurrentMonth && "text-white/20",
                  )}>
                    {format(day, "d")}
                  </span>
                  <div className="mt-1 space-y-0.5">
                    {dayReservations.slice(0, 2).map(r => (
                      <div key={r.id} className={cn(
                        ...
                        STATUS_COLOR[r.statut]?.pill ?? "bg-white/5 text-white/40"
                      )}>
                        <span className={cn("w-1 h-1 rounded-full flex-shrink-0", STATUS_COLOR[r.statut]?.dot ?? "bg-white/20")} />
                        <span className="truncate">{r.vehicule.immatriculation}</span>
                      </div>
                    ))}
                    {dayReservations.length > 2 && (
                      <p className="text-[10px] text-white/30 px-1">+{dayReservations.length - 2} autre{dayReservations.length - 2 > 1 ? "s" : ""}</p>
                    )}
                  </div>
                </button>
              );
        ...
        <div className="surface p-5 flex flex-col">
          {selectedDay ? (
            <div className="flex-1">
              <p className="text-xs font-semibold text-white/40 uppercase tracking-widest mb-1">Sélectionné</p>
              <p className="text-sm font-bold text-white/90 mb-4 capitalize">
                {format(selectedDay, "EEEE d MMMM", { locale: fr })}
              </p>
              {selectedDayReservations.length === 0 ? (
                <div className="text-center py-10">
                  <CalendarDays className="w-8 h-8 text-white/10 mx-auto mb-2" />
                  <p className="text-sm text-white/25">Aucune réservation ce jour</p>
                </div>
              ) : (
                <div className="space-y-2.5">
                  {selectedDayReservations.map(r => (
                    <div key={r.id} className="p-3.5 bg-white/[0.03] border border-white/[0.06] rounded-xl hover:border-white/10 transition-colors">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="text-sm font-semibold text-white/90">{r.client.prenom} {r.client.nom}</p>
                          <p className="text-xs text-white/40 mt-0.5">{r.vehicule.marque} {r.vehicule.modele}</p>
                        </div>
                      ...
                      <code className="text-[11px] font-mono text-white/25">{r.vehicule.immatriculation}</code>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-8">
              ...
              <p className="text-sm text-white/25 text-center">Cliquez sur un jour pour voir les réservations</p>
            </div>
          )}

          {/* Legend */}
          <div className="mt-auto pt-4 border-t border-white/[0.06]">
            <p className="text-[10px] font-semibold text-white/25 uppercase tracking-widest mb-3">Légende</p>
            <div className="space-y-2">
              {Object.entries(STATUT_RESERVATION_LABELS)
                .filter(([k]) => k !== "ANNULEE")
                .map(([key, label]) => (
                  <div key={key} className="flex items-center gap-2.5">
                    <span className={cn("w-2 h-2 rounded-full flex-shrink-0", STATUS_COLOR[key]?.dot ?? "bg-white/20")} />
